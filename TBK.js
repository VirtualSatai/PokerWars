...
const RAISE_RANGE_INDEX = 8;
const CALL_RANGE_INDEX = 14;
const BIG_BLINDS_TO_ALL_IN_RATIO = 4;
const BIG_BLINDS_LEFT_BEFORE_TRYING_ALL_IN_ON_FLOP = 15;
  ...
  function update(game) {
    if (game.state !== "complete") {
      if (game.state === "pre-flop") {
        ...
        if (game.self.chips <= currentBigBlind * BIG_BLINDS_LEFT_BEFORE_TRYING_ALL_IN_ON_FLOP) {
        ...
        if (hand.rank >= 5) {
          ...
          return Math.max(getBigBlind(game) * 3, game.betting.raise * 1.5);
        } else if (hand.rank > 2) {
          return game.betting.raise;
        } else if (hand.rank === 2) {
          return game.betting.call;
