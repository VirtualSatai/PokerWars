...
module.exports = function () {
    ...
    if (game.state !== "complete") {
      ...
      if(game.state === "pre-flop"){
        ...
        if(Balzyness == 1){
          ...
          var raiseRange = allRanges[Math.floor(50+WinState)];
          ...
          var raiseRange = allRanges[Math.floor(25*WinState)];
          var callRange = allRanges[Math.floor(50+WinState)];
          if(raiseRange.isHandInRange(game.self.cards)){
            return game.betting.raise;
        ...
        if(allcards.rank >= AgressiveVar+1){
          return game.self.chips;
        }
        if(allcards.rank >=AgressiveVar){
          if(allcards.rank === 2){
            if(Hand.solve(game.self.cards).rank <2){
              return 0;
            }
          }
  ...
  function validateblind(game){
    ...
    if(bigblind*3*BallzyVar >= game.self.chips){
      return 1;
    }else if(bigblind*6*BallzyVar >= game.self.chips){
      return 2;
    }else if(bigblind * 10*BallzyVar>= game.self.chips){
      return 3;
    }else{return 4;}
  }

  function goodcardsFlop(game){
    Cards_p3 = allRanges[Math.floor(3+WinState)]
    Cards_p10 = allRanges[Math.floor(10+WinState)]
    Cards_p20 = allRanges[Math.floor(20+WinState)]
