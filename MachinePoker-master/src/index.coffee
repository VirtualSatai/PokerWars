...
class MachinePoker extends EventEmitter
  ...
  addPlayers: (bots) ->
    ...
    for bot in bots
      player = bot.player
      name = botNameCollision(names, player.name)
      names.push(name)
      if ("chips" of bot) 
        @addPlayer(new Player(player, bot.chips, name))
      else
        @addPlayer(new Player(player, @chips, name))

  addPlayer: (player, chips) ->
    addPlayer(new Player(player, chips ))
